    ...
    def file_check(self,dest_folder):
        try:
            ...
            if n != 0:
                ...
                extractor.download(links, rf'C:\Users\hamed\Downloads\{self.nickname}\{self.keyword}')
    ...
    try:
        ...
        if use_dl_list == 'yes':
            with open('dl_list.txt', 'r') as dl_file:
                for line in dl_file:
                    ...
                    if len(parts) == 3:
                        ...
                        extractor.run(os.getenv('EMAIL'), os.getenv('PASSWORD'), epi)
                        extractor.download(rf'.\temp\{nickname}_links.txt', rf'C:\Users\hamed\Downloads\{nickname}\{keyword}')
                        extractor.retry(fr'C:\Users\hamed\Downloads\{nickname}\{keyword}')
                        extractor.file_check(rf'C:\Users\hamed\Downloads\{nickname}\{keyword}')
                        extractor.update_dl_list(keyword, epi)
                    elif len(parts) == 2:
                        keyword, epi = parts
                        extractor = DownloadLinkExtractor(keyword, keyword, 3)
                        extractor.run(os.getenv('EMAIL'), os.getenv('PASSWORD'), epi)
                        extractor.download(rf'.\temp\{keyword}_links.txt', fr'C:\Users\hamed\Downloads\{keyword}')
                        extractor.retry(fr'C:\Users\hamed\Downloads\{keyword}')
                        extractor.file_check(fr'C:\Users\hamed\Downloads\{keyword}')
                        extractor.update_dl_list(keyword, epi)
                    else:
            ...
            if not email or not password:
                ...
                extractor.download(rf'.\temp\{nickname}_links.txt', rf'C:\Users\hamed\Downloads\{nickname}\{keyword}')
                extractor.retry(fr'C:\Users\hamed\Downloads\{nickname}\{keyword}')
                extractor.file_check(fr'C:\Users\hamed\Downloads\{nickname}\{keyword}')
                extractor.update_dl_list(keyword, epi) 
            print("All donwloads complete")
        else:
